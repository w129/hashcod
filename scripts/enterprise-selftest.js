...
(async () => {
  ...
  assert(blindSpotSource.includes('Blind spot shell') && blindSpotSource.includes('navigator.clipboard.readText') && blindSpotSource.includes("indexedDB.open(DB_NAME") && blindSpotSource.includes("serviceWorker.register('/blind-spot-sw.js?v=access-qr-3')"), 'blind-spot-shell-offline-office');
  ...
  assert(blindSpotSwSource.includes('blind-spot-shell-v3') && blindSpotSwSource.includes('/blind-spot-shell.html') && blindSpotSwSource.includes('/app/hashcod-platform-icon.svg') && blindSpotSwSource.includes('/vendor/qrcode.min.js') && blindSpotSwSource.includes('fetch(event.request).then'), 'blind-spot-shell-service-worker');
