...
(async () => {
  ...
  assert(blindSpotSwSource.includes('blind-spot-shell-v5') && blindSpotSwSource.includes('/blind-spot-shell.html') && blindSpotSwSource.includes('/app/hashcod-platform-icon.svg') && blindSpotSwSource.includes('/vendor/qrcode.min.js') && blindSpotSwSource.includes('notificationclick') && blindSpotSwSource.includes('fetch(event.request).then'), 'blind-spot-shell-service-worker');
